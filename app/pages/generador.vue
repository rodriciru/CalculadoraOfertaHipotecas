      ...
      <UCard :ui="{ ring: 'ring-1 ring-gray-200 dark:ring-gray-700', shadow: 'shadow-lg' }">
        <template #header>
          <div class="flex justify-between items-center">
            ...
            <UButton icon="i-heroicons-x-mark" color="neutral" variant="ghost" @click="closeEditor" />
          </div>
        </template>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div class="md:col-span-4 space-y-4">
             ...
             <UFormField label="Tipo de interés">
                <USelect v-model="editor.data.tipo" :items="['fija', 'variable', 'mixta']" />
             </UFormField>
          </div>
          ...
          <div class="md:col-span-4">
              <div class="flex justify-between mb-2">
                 <h4 class="font-medium text-sm text-gray-500">Gastos Adicionales (€)</h4>
              </div>
              ...
              <div class="flex gap-2 items-center mb-4">
                ...
                <UInput
                  ...
                  size="sm"
                />
                <UButton icon="i-heroicons-plus" color="success" size="sm" @click="addPredefinedGasto">Añadir</UButton>
              </div>

              <!-- Añadir Gasto Personalizado -->
              <div class="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
                <h5 class="font-medium text-xs text-gray-500 mb-2">Añadir Gasto al Catálogo</h5>
                <div class="flex gap-2 items-center mb-2">
                  <UInput v-model="nuevoGastoPersonalizadoNombre" placeholder="Nombre nuevo gasto" class="w-full" size="sm" />
                  <UInput v-model.number="nuevoGastoPersonalizadoCoste" type="number" placeholder="Coste (€)" class="w-24" size="sm" />
                  <UButton icon="i-heroicons-plus" color="success" size="sm" @click="addGastoPersonalizado">Crear y Añadir</UButton>
                </div>
              </div>

              <div v-if="editor.data.gastosAdicionales?.length" class="space-y-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                 <div v-for="(gasto, gIndex) in editor.data.gastosAdicionales" :key="gIndex" class="flex gap-2 items-center">
                    <UInput v-model="gasto.nombre" placeholder="Nombre del gasto" class="w-full" size="sm" />
                    <UInput v-model.number="gasto.coste" placeholder="Coste €" class="w-24" type="number" size="sm" />
                    <UButton icon="i-heroicons-trash" color="error" variant="ghost" @click="removeGasto(gIndex)" />
                 </div>
              </div>
              <p v-else class="text-xs text-gray-400 italic mt-4">No hay gastos adicionales en esta oferta.</p>
          </div>
          ...
          <div class="md:col-span-8">
              ...
              <div class="space-y-2 mb-4 p-3 border rounded-md bg-gray-50 dark:bg-gray-800">
                <h5 class="font-medium text-xs text-gray-500 mb-2">Seleccionar del Catálogo</h5>
                <UFormField label="Bonificación del Catálogo" class="mb-2">
                  <USelect
                ...
                <div class="grid grid-cols-2 gap-2 mb-2">
                  <UFormField label="Coste Anual (€)">
                    <UInput v-model.number="costoBonificacionAAnadir" type="number" step="10" size="sm" />
                  </UFormField>
                  <UFormField label="Red. TIN (%)">
                    <UInput v-model.number="reduccionTinBonificacionAAnadir" type="number" step="0.01" size="sm" />
                  </UFormField>
                  <UFormField label="Red. Diferencial (%)">
                    <UInput v-model.number="reduccionDiferencialBonificacionAAnadir" type="number" step="0.01" size="sm" />
                  </UFormField>
                  <UFormField label="Red. TIN Fijo (%)">
                    <UInput v-model.number="reduccionTinFijoBonificacionAAnadir" type="number" step="0.01" size="sm" />
                  </UFormField>
                </div>
                <UCheckbox v-model="isSupposedBonificacionAAnadir" label="Valor supuesto" class="mb-2" />
                <UButton icon="i-heroicons-plus" color="success" size="sm" @click="addBonificacionPredefinida">Añadir Bonificación</UButton>
              </div>
              ...
              <div class="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4 space-y-2 p-3 rounded-md bg-gray-50 dark:bg-gray-800">
                <h5 class="font-medium text-xs text-gray-500 mb-2">Crear y Añadir al Catálogo</h5>
                <UFormField label="Nombre Nueva Bonificación" class="mb-2">
                  <UInput v-model="nombreBonificacionPersonalizada" placeholder="Nombre de la bonificación" size="sm" />
                </UFormField>
                <div class="grid grid-cols-2 gap-2 mb-2">
                  <UFormField label="Coste Anual (€)">
                    <UInput v-model.number="costeAnualBonificacionPersonalizada" type="number" step="10" size="sm" />
                  </UFormField>
                  <UFormField label="Red. TIN (%)">
                    <UInput v-model.number="reduccionTinBonificacionPersonalizada" type="number" step="0.01" size="sm" />
                  </UFormField>
                  <UFormField label="Red. Diferencial (%)">
                    <UInput v-model.number="reduccionDiferencialBonificacionPersonalizada" type="number" step="0.01" size="sm" />
                  </UFormField>
                  <UFormField label="Red. TIN Fijo (%)">
                    <UInput v-model.number="reduccionTinFijoBonificacionPersonalizada" type="number" step="0.01" size="sm" />
                  </UFormField>
                </div>
                <UCheckbox v-model="isSupposedBonificacionPersonalizada" label="Valor supuesto" class="mb-2" />
                <UButton icon="i-heroicons-plus" color="success" size="sm" @click="addBonificacionPersonalizada">Crear y Añadir</UButton>
              </div>

              <!-- Bonificaciones en esta oferta -->
              <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                <h5 class="font-medium text-sm text-gray-500 mb-2">Bonificaciones en esta Oferta</h5>
                <div v-if="editor.data.bonificaciones?.length" class="space-y-2">
                  <div v-for="(bon, bIndex) in editor.data.bonificaciones" :key="bon.id || bIndex" class="flex flex-wrap items-center gap-x-2 gap-y-1">
                    ...
                    <span v-if="bon.reduccionTin" class="text-xs text-blue-600 dark:text-blue-300">-{{ bon.reduccionTin.toFixed(2) }}% TIN</span>
                    <span v-if="bon.reduccionDiferencial" class="text-xs text-green-600 dark:text-green-300">-{{ bon.reduccionDiferencial.toFixed(2) }}% Dif.</span>
                    <span v-if="bon.reduccionTinFijo" class="text-xs text-purple-600 dark:text-purple-300">-{{ bon.reduccionTinFijo.toFixed(2) }}% TIN Fijo</span>
                    <span v-if="bon.isSupposed" class="text-xs text-yellow-600 dark:text-yellow-300">(Valor supuesto)</span>
                    <UButton icon="i-heroicons-trash" color="red" variant="ghost" size="xs" @click="removeBonif(bIndex)" class="ml-auto" />
                  </div>
                </div>
                <p v-else class="text-xs text-gray-400 italic">No hay bonificaciones añadidas a esta oferta.</p>
              </div>
          </div>
        ...
        <template #footer>
          <div class="flex justify-end gap-3">
             <UButton label="Cancelar" color="neutral" @click="closeEditor" />
             <UButton label="Guardar Cambios" color="success" icon="i-heroicons-check" @click="saveForm" />
          </div>
        </template>

...
export class CalculadoraHipoteca {
  ...
  static obtenerInteresMensual(oferta: OfertaHipotecaTipo, euribor: number, conBonificaciones: boolean, mesActual: number): { rate: number, isAssumedBase: boolean } {
    let baseRate = 0
  ...
  static calcularTAE(prestamo: number, numCuotas: number, gastosIniciales: number, flujoCuotas: number[], costeMensualProductos: number): number {
  ...
  static calcularFlujoDeCuotas(oferta: OfertaHipotecaTipo, importe: number, plazoMeses: number, euribor: number, conBonificaciones: boolean): number[] {
    let capitalPendiente = importe
    const cuotas: number[] = []

    if (oferta.tipo === 'fija') {
      const interesMensual = this.obtenerInteresMensual(oferta, euribor, conBonificaciones, 1)
      const cuotaConstante = this.calcularCuotaFrances(importe, interesMensual.rate, plazoMeses)
      return Array(plazoMeses).fill(cuotaConstante)
    }

    if (oferta.tipo === 'variable') {
      for (let mes = 1; mes <= plazoMeses; mes++) {
        const interesMensual = this.obtenerInteresMensual(oferta, euribor, conBonificaciones, mes)
        const cuota = this.calcularCuotaFrances(capitalPendiente, interesMensual.rate, plazoMeses - mes + 1)
    ...
    if (oferta.tipo === 'mixta') {
      ...
      const interesFijoMensual = this.obtenerInteresMensual(oferta, euribor, conBonificaciones, 1)
      const cuotaFija = this.calcularCuotaFrances(importe, interesFijoMensual.rate, plazoMeses)
      ...
      for (let mes = plazoFijoMeses + 1; mes <= plazoMeses; mes++) {
        const interesVariableMensual = this.obtenerInteresMensual(oferta, euribor, conBonificaciones, mes)
        const cuotaVariable = this.calcularCuotaFrances(capitalPendiente, interesVariableMensual.rate, plazoMeses - mes + 1)
  ...
  static calcularResultadoParaOferta(oferta: OfertaHipotecaTipo, importe: number, plazoAnios: number, euribor: number, personalBonificaciones: IPersonalBonus[]): IResultadoCalculo {
    ...
    const baseRateInfoConBonif = this.obtenerInteresMensual(oferta, euribor, true, 1)

    const cuotasSinBonificar = this.calcularFlujoDeCuotas(oferta, importe, plazoMeses, euribor, false)
    ...
    const taeSinBonificar = this.calcularTAE(importe, plazoMeses, gastosParaTAE, cuotasSinBonificar, 0)

    const cuotasConBonificar = this.calcularFlujoDeCuotas(oferta, importe, plazoMeses, euribor, true)
    ...
    const taeConBonificar = this.calcularTAE(importe, plazoMeses, gastosParaTAE, cuotasConBonificar, costeTotalBonificaciones / plazoMeses)
    ...
    if (oferta.tipo === 'fija') {
      ...
      const plazoFijoMeses = oferta.plazoFijoAnios * 12
      const baseRateInfoVariableSinBonif = this.obtenerInteresMensual(oferta, euribor, false, plazoFijoMeses + 1)
      const baseRateInfoVariableConBonif = this.obtenerInteresMensual(oferta, euribor, true, plazoFijoMeses + 1)
